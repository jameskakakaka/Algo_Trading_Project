...
1. **Data Ingestion Layer:** Fetches raw OHLCV market data from exchange APIs with strict type casting.

2. **Streaming Pipeline (Apache Kafka):** Acts as the central nervous system, decoupling data producers from consumers to handle high-frequency market volatility.

3. **Storage Engine (Custom LSM-Tree):** A highly efficient database engine built from scratch featuring:
...
4. **AI Brain (Machine Learning):** Real-time feature engineering (RSI, MACD, Bollinger Bands, EMA, ATR) combined with an XGBoost classifier trained on historical data to predict price direction.

5. **Execution Engine:** An event-driven paper-trading bot that consumes predictions and executes simulated trades while managing state and risk.
